...
By replacing code below to ``bsp.go``. We can measure the blocking time.
...
func bspBlocking(tid int, numOfThreads int,
	...
	cond.L.Unlock()
	if start != nil {
		blockingDuration += time.Since(*start).Seconds()
	}
}
...
func executeBSP(tid int, ctx *bspContext) {
	...
	for {
		...
		if ctx.done {
			...
			if atomic.AddInt32(ctx.exitCounter, 1) == int32(ctx.numOfThreads-1) {
				fmt.Fprintf(os.Stderr, "Blocking: %f\n", blockingDuration)
				fmt.Fprintf(os.Stderr, "Non-Blocking: %f\n", time.Since(globalStart).Seconds()-blockingDuration)
			}
